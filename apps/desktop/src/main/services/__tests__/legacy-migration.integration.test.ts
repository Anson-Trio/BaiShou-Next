...
describe.skipIf(!isBetterSqlite3Available())('LegacyMigrationService Integration Tests', () => {
  ...
  const prepareEnv = async (testName: string) => {
    ...
    if (fs.existsSync(currentMockUserData)) {
      try {
        await fsp.rm(currentMockUserData, { recursive: true, force: true })
      } catch {}
    }
  ...
  afterEach(async () => {
    if (realDbInstance && (realDbInstance as any).session?.client) {
      try {
        await ((realDbInstance as any).session.client as any).close()
      } catch {}
    }
  })
  ...
  it('Case 1: Fresh legacy install with only config and empty DB', async () => {
    ...
    const aiProv = await realDbInstance
      .select()
      .from(systemSettingsTable)
      .where(eq(systemSettingsTable.key, 'ai_providers'))
      .get()
  ...
  it('Case 3: Legacy DB with shadow_index is preserved physically', async () => {
    ...
    expect(fs.existsSync(path.join(targetDir, 'SecretVault', '.baishou', 'shadow_index.db'))).toBe(
      true
    )
  })
