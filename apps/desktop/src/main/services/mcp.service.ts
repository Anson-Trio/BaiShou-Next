...
import type { DesktopMcpConfigReader } from './desktop-mcp-config.store'
...
export class McpService {
  ...
  constructor(
    private readonly configReader: DesktopMcpConfigReader,
  ...
  private async ensureAuthorized(req: express.Request, res: express.Response): Promise<boolean> {
    const config = await this.configReader.getMcpServerConfig()
    ...
    return new Promise((resolve, reject) => {
      try {
        this.httpServer = this.app.listen(port, '127.0.0.1', () => {
          ...
          logger.info(`[McpService] Server started on http://127.0.0.1:${actualPort}/mcp`)
          resolve()
        })
