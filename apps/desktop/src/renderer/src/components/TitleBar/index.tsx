    ...
    <div className={`${styles.titlebar} ${platform === 'darwin' ? styles.mac : styles.win}`}>
      ...
      {platform === 'win32' && (
        <div className={styles.windowControls}>
          <button className={styles.controlBtn} onClick={handleMinimize}>&#xE921;</button>
          <button className={styles.controlBtn} onClick={handleMaximize}>&#xE922;</button>
          <button className={`${styles.controlBtn} ${styles.closeBtn}`} onClick={handleClose}>&#xE8BB;</button>
        </div>
      )}
    </div>
  );
