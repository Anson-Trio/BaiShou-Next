...
function processFile(filePath) {
  ...
  content = content.replace(
    /background(-color)?:\s*var\(--color-surface[^)]*\)(?:,\s*(#[0-9a-fA-F]+|rgba?\([^)]+\)))?;/g,
    'background: var(--bg-surface);'
  )
  content = content.replace(
    /background(-color)?:\s*var\(--color-surface-variant[^)]*\)(?:,\s*(#[0-9a-fA-F]+|rgba?\([^)]+\)))?;/g,
    'background: var(--bg-surface-normal);'
  )
  content = content.replace(
    /background(-color)?:\s*var\(--color-surface-container[^)]*\)(?:,\s*(#[0-9a-fA-F]+|rgba?\([^)]+\)))?;/g,
    'background: var(--bg-surface-normal);'
  )
  content = content.replace(
    /background(-color)?:\s*var\(--color-surface-dark[^)]*\)(?:,\s*(#[0-9a-fA-F]+|rgba?\([^)]+\)))?;/g,
    'background: var(--bg-surface);'
  )

  // Replace hardcoded rgba(255, 255, 255, x) backgrounds
  content = content.replace(
    /background(-color)?:\s*rgba?\(\s*255\s*,\s*255\s*,\s*255\s*,\s*[\d.]+\s*\);/g,
    'background: var(--bg-glass-surface);'
  )

  // Replace missing text colors
  content = content.replace(
    /color:\s*var\(--color-on-surface[^)]*\)(?:,\s*(#[0-9a-fA-F]+|rgba?\([^)]+\)))?;/g,
    'color: var(--text-primary);'
  )
  content = content.replace(
    /color:\s*var\(--color-on-surface-variant[^)]*\)(?:,\s*(#[0-9a-fA-F]+|rgba?\([^)]+\)))?;/g,
