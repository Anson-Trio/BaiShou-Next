...
function processFile(filePath) {
  ...
  content = content.replace(
    /background(-color)?:\s*var\(--color-surface[^)]*\)(?:,\s*#[0-9a-fA-F]+)?;/g,
    'background: var(--bg-surface);'
  )
  content = content.replace(
    /background(-color)?:\s*var\(--color-surface-variant[^)]*\)(?:,\s*#[0-9a-fA-F]+)?;/g,
    'background: var(--bg-surface-normal);'
  )
  content = content.replace(
    /background(-color)?:\s*var\(--color-surface-container[^)]*\)(?:,\s*#[0-9a-fA-F]+)?;/g,
    'background: var(--bg-surface-normal);'
  )

  // Replace missing text colors
  content = content.replace(
    /color:\s*var\(--color-on-surface[^)]*\)(?:,\s*#[0-9a-fA-F]+)?;/g,
    'color: var(--text-primary);'
  )
  content = content.replace(
    /color:\s*var\(--color-on-surface-variant[^)]*\)(?:,\s*#[0-9a-fA-F]+)?;/g,
  ...
  if (filePath.includes('DiaryPage.css')) {
    content = content.replace(
      /background-color:\s*rgba\(255,\s*255,\s*255,\s*0\.8\);/g,
