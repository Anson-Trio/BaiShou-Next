...
export interface SessionCompressionConfig {
  ...
  force?: boolean
  /** 模型上下文窗口（token）；用于按窗口触发压缩，对齐 opencode model.limit.context */
  modelContextWindow?: number
  /** 为输出与系统提示预留的 token；usable = window - reserved */
  reservedTokens?: number
  /** 保留区 token 预算（对齐 opencode preserve_recent_tokens） */
...
/**
 * 计算压缩后「保留区起点」消息 id（opencode tail_start_id 等价物）。
...
/**
 * 按「保留最近 N 轮用户对话」+ 可选 token 预算切分（对齐 opencode tail_turns + preserve_recent_tokens）
 */
