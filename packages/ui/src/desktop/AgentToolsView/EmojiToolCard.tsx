...
export const EmojiToolCard: React.FC<EmojiToolCardProps> = ({ config, onChange }) => {
  ...
  const probabilityPercent = Math.round((emojiConfig.replyProbability ?? 0.3) * 100)
  ...
  const handleProbabilityChange = (value: number) => {
    onChange({
      ...config,
      emojiConfig: { ...emojiConfig, replyProbability: Math.min(1, Math.max(0, value)) }
    })
  }
      ...
      >
        <div className={styles.emojiPopupContent}>
          ...
          <div className={styles.emojiPopupRow}>
            <span className={styles.emojiPopupLabel}>
              {t('agent.tools.emoji_reply_probability', '回复概率')}
            </span>
            <div className={styles.emojiPopupSliderControl}>
              <input
                type="range"
                min={0}
                max={100}
                value={probabilityPercent}
                onChange={(e) => handleProbabilityChange(Number(e.target.value) / 100)}
                className={styles.emojiRangeInput}
                style={{ backgroundSize: `${probabilityPercent}% 100%` }}
              />
              <span className={styles.emojiPopupSliderValue}>{probabilityPercent}%</span>
            </div>
          </div>
          ...
          <div className={styles.emojiManageSection}>
            ...
            {(!emojiConfig.emojis || emojiConfig.emojis.length === 0) && (
              <div className={styles.emojiEmptyHint}>
          ...
          {/* 作者署名 */}
          <div className={styles.emojiPopupAuthor}>Creator: Ratman463</div>
        </div>
      </Modal>
    </div>
  )
