...
const ToolResultItem: React.FC<{ invocation: MockToolInvocation }> = ({ invocation }) => {
  ...
  const rawContent =
    typeof invocation.result === 'string' ? invocation.result : JSON.stringify(resultObj)
  const isError =
    rawContent.startsWith('Error') ||
    rawContent.startsWith('Tool execution failed:') ||
    rawContent.toLowerCase().includes('failed')
  ...
  return (
    <div
      className={`${styles.itemCard} ${isError ? styles.itemError : ''} ${expanded ? styles.itemOpen : ''}`}
    >
      <div className={styles.itemHeader} onClick={() => setExpanded(!expanded)}>
        <span className={styles.itemStatusWrap}>
          {isError ? (
            <XCircle size={14} color="rgba(244, 67, 54, 1)" />
          ) : (
        ...
        <div className={styles.itemContentInner}>
          <div className={styles.contentWrapper}>
            {parsedJson && !isError ? (
              ...
              <pre className={`${styles.resultTextLog} ${isError ? styles.errorText : ''}`}>
                {rawContent}
              </pre>
            )}
          </div>
